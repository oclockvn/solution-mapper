...
public static class WinMergeProjectFile
{
    ...
    public static string Build(IReadOnlyList<DiffPair> pairs, string? filter = null)
    {
        ...
        foreach (var p in pairs)
        {
            root.Add(new XElement("paths",
                ...
                new XElement("left-readonly", 1),
                new XElement("right-readonly", 1),
    ...
    public static string Write(IReadOnlyList<DiffPair> pairs, string? filter = null)
    {
        ...
        File.WriteAllText(path, Build(pairs, filter), new UTF8Encoding(false));
        return path;
    }
}
